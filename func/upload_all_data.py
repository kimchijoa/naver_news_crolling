...
def search_and_insert(dirname):
    ...
    for i in range(len(total_news_xls_path)):
        ...
        #s3에 파일 업로드 하면서 s3 파일 정보 sql에 저장
        ...
        #엑셀 파일 정보 저장
        sql_cursor, conn = m_sql.create_conn()
        m_sql.crolling_speed_info(conn, sql_cursor, "today_crolling_speed", folder_name + graph_speed_info_file, str(folder_date_list[i]))
